...
const replaceWords = (str) => {
    ...
    for (let i = 0; i < words.length - 1; i++) {
        if (words[i] == 'I') {
            let indexI = words.indexOf("I");
            words.splice(indexI, 1, 'We');

        } else if (words[i] == 'am') {
            let indexAm = words.indexOf("am");
            words.splice(indexAm, 1, 'are');

        } else if (words[i] == 'was') {
            let indexWas = words.indexOf("was");
